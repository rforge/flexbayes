...
/*
  ...
  |
  |  lambda_ij ~ Gamma( xi_j, (xi_j / mu_ij) )
  |
  |  log( mu_ij ) = X_ij beta_j + M_ij gamma
  |
                     ...
                     double * mh_drawing_stats ) throw( rtErr ) 
{
  ...
  BayesianPoissonLogLinkModel bayes_PoissonLL;
  ...
  if ( ((int) (*exposures_flag)) > 0 )
  {
    ...
    bayes_PoissonLL.initialize( counts, exposures, ((int) (*number_groups) ) );
  }
  else
  {
  	// Similar, without setting the exposures data.
    bayes_PoissonLL.initialize( counts, ((int) (*number_groups) ) );
  }  
  ...
  {
    bayes_PoissonLL.xiType( 0 );   // group-specific overdispersion parameter case
  ...
  {
    bayes_PoissonLL.xiType( 1 );   // common overdispersion parameter case
  } 
  else {
  	bayes_PoissonLL.xiType( 2 );  // no overdispersion case
  }
  	   ...
  	   ( ( (int) (*common_xi) ) != 3 ) )  // overdispersion parameter case
  {
    ...
    {
      bayes_PoissonLL.samplerXiInitialPoint( (*xiInit) );
    }
    else
    {
#ifdef DEBUG1
  ...
  {
    bayes_PoissonLL.samplerDefaultInitialPoint();
  }


  //connect both parts
  bayes_HGlm.glmModel( &bayes_PoissonLL );
