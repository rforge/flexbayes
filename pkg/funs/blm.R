...
blm <- function( formula, data, prior = blm.prior(), likelihood = blm.likelihood(),
	         sampler = blm.sampler(), random.seed = .Random.seed,
	         na.action = na.fail, contrasts = NULL)
{
  ...
  model.blm <- eval( model.blm, sys.parent() )
  ...
  if ( class( prior$priorBeta ) == "bayes.distribution" )
  {
  ...
  if ( class( prior$priorSigma ) == "bayes.distribution" )
  {
  ...
  iType <- switch( iType,
		...
		"normal:normal:non-informative" = {
			betaMean <- prior$priorBeta@parameters[["mean vector"]]
			betaCov <- prior$priorBeta@parameters[["covariance matrix"]]
			betaDf <- 3.0
			sigmaDf <- 0.0
			sigmaScale <- 1.0
			1
		...
		"normal:t:non-informative" = {
			betaMean <- prior$priorBeta@parameters[["mean vector"]]
			betaCov <- prior$priorBeta@parameters[["covariance matrix"]]
			betaDf <- prior$priorBeta@parameters[["t degrees of freedom"]]
			sigmaDf <- 0.0
			sigmaScale <- 1.0
			2
		},
...
###CHANGE GUI TO ASK FOR VARIANCE
  ...
  {
    prior$conjugate <- F
    warning( "blm: prior has been set up as conjugate, but priors provided do not match the conjugate type.\nPrior has been modified to non-conjugate type.\n" )
  }
  ...
  if ( !is.element( iType, c( 7, 8, 9, 10, 11, 12, 13, 14, 15 ) ) ) 
  {
    if ( length( betaMean ) != dim.Cov )
    {
      stop( "blm: Wrong dimension for coefficients mean prior. It is ", length( betaMean ), " and should be ", dim.Cov, "\n" )
    }
  }
  ...
  if ( is.null( init.point ) || init.point$type == "prior" )
  {
    # flag for prior means starting points
      ...
      starting.points <- generateInitPoints.blm( number.chains, X, Y, sigmaDf, sigmaScale, 
                                                 betaMean, betaCov, betaDf, beta.components, beta.props, 
                                                 mix.with.MLE = 0 )
  ...
  if ( class( prior$priorBeta ) == "bayes.distribution" )
  {
    prior$priorBeta@parameters[["mean vector"]] <- betaMean;
    prior$priorBeta@parameters[["covariance matrix"]] <- betaCov
    if ( is.element( prior$priorBeta@name, c( "normal mixture", "t mixture" ) ) )
    {
      prior$priorBeta@parameters[["props"]] <- beta.props
      if ( prior$priorBeta@name == "t mixture" )
      {
        prior$priorBeta@parameters[["t degrees of freedom"]] <- betaDf
      }
    }
  }
  else
  ...
  if ( location.scale && length( betaMean ) == 1 )
  {
    #change name of "intercept" to "mean"
    beta.names <- "MEAN"
  }
  ...
  for ( i in seq( 1, number.chains ) )
  {
    ...
    fit <- fit.bayeslm( X, Y, likelihood$errorCov, degreesOfFreedom.likelihood, sigmaDf, sigmaScale,
	            iType, betaMean, betaCov, betaDf, beta.props, beta.components,
                    burnInLength, simulationsToPerform, sampleFrequency, 
                    sampler.type, read.init.point, starting.points$beta[ ,i ], starting.points$sigma[i],
                    print.stats )
    ...
    if ( length( fit$mixture ) > 1 )
    {
      ...
      if ( sum.mixture > 0 )
      {
        ...
        dimnames( mixture.drawing.stats ) <- list( paste( "Component:", seq( 1, beta.components ), sep = "" ), c( "Proportion", "Count" ) )
      }
    }
	  ...
	  if ( is.element( iType, c( 3, 4, 5, 11, 12, 13, 14 ) ) )
    {
    	...
    	dimnames(samples.this.chain) <- list(NULL,c(beta.names, "SIGMA", tau2ErrorScale.names))
  		...
  		blmodel[[i]] <- mcmc(samples.this.chain, thin=sampleFrequency, 
  			start=(burnInLength+1), end=(simulationsToPerform*sampleFrequency+burnInLength))
    }
    else
    {
	...
	return( posterior( sims = mcmc.list(blmodel), call = match.call() ) )
}#end blm generator


fit.bayeslm <- function(X, Y, errorCov, degreesOfFreedom.likelihood, sigmaDf,
	sigmaScale, iType, betaMean, betaCov, betaDf, props = 1, n.mixtures = 1, 
        burnInLength,
	simulationsToPerform, sampleFrequency, sampler.type = 0,
        read.init.point = 0, beta.init, sigma.init, print.stats = 0 )
{
	...
	dim.error.Cov <- length(errorCov)
        ...
        if (  is.element( iType, c( 2, 5 ) ) )
        {
          #account for tau2 for beta t-prior
	...
	# (= sqrt(sigma2))
	  ...
	  out.samples <- matrix( fit$output.samples, nrow = simulationsToPerform, ncol = dim.Cov + 1 + number.data )
        ...
        list( samples = out.samples, gibbs = fit$gibbs.stats, mixture = fit$mixture.stats )
...
blm.likelihood <- function( type = "normal",
                            df = 3 )
{
...
blm.prior <- function( fixed.coef = "non-informative", 
                       sigma2 = "non-informative")
{
	conjugate <- F 
	priorSigma <- sigma2
	priorBeta <- fixed.coef
  
  if ( class( priorBeta ) != "bayes.distribution" && priorBeta != "non-informative" )
  {
    stop( "blm.prior: prior for coefficients is not of the right type.\n" )
  }
  else if ( class( priorBeta ) == "bayes.distribution" )
  {
    if ( !is.element( priorBeta@name, c( "normal", "t", "normal mixture", "t mixture" ) ) )
    {
      stop( "blm.prior: prior specification for coefficients is not supported." )
    }
  }

  if ( class( priorSigma ) != "bayes.distribution" && priorSigma != "non-informative" )
  {
    stop( "blm.prior: prior for variance is not of the right type.\n" )
  }
  else if ( class( priorSigma ) == "bayes.distribution" )
  {
...
blm.sampler <- function( nBurnin = 1000, nSamples = 1000, nThin = 1,
                         nChains = 1,
                         init.point = "prior + likelihood",
                         gamma.init = NULL,
                         sigma2.init = NULL )
{
  ...
  if ( number.chains <= 0 )
  {
    warning( "blm.sampler: zero or negative number of chains to simulate. Assuming 1 chain." )
    number.chains <- 1
  }
  ...
  if ( ceiling( number.chains ) - number.chains > 0 )
  {
    ...
    warning( "blm.sampler: Non-integer number of chains specified [", number.chains, "]. Set number of chains to [", new.number.chains, "]." )
  ...
  init.point <- match.arg( init.point, c( "prior", "prior + likelihood", "likelihood", "user's choice" ) )
  ...
  list( control = blm.control(nBurnin=nBurnin, nSamples=nSamples, nThin=nThin),
