...
#        Bayes Linear Model)
#
init.points.blm <- function( formula, data, prior = blm.prior(), number.draws = 1,
	na.action = na.fail, contrasts = NULL, ...)
{
	...
	iType <- switch( iType,
		...
		"normal:invChisq" = {
			...
			sigmaScale <- prior$priorSigma@parameters[[ "sigma0.sq" ]]
                        ...
                        #cat(" sigmaDf is ", sigmaDf, "  sigmaScale is ", sigmaScale, "\n" )

			ifelse( prior$conjugate, 6, 1 )
		},


		"normal mixture:non-informative" = {
			betaMean <- prior$priorBeta@parameters[["mean vector"]]
			betaCov <- prior$priorBeta@parameters[["covariance matrix"]]
        ...
        if ( is.element( iType, c( 7, 8, 9, 10 ) ) ) 
        {
          ...
          if ( beta.k > 0 && beta.components == 2 )
          {
            ...
            if ( is.vector( betaMean ) )
            {
              ...
              {
                stop( "blm: problem with prior mean. mixture prior for the coefficients is not valid." )
              }
            }
            ...
            #cat( "init: number components is ", beta.components, ".  props are\n" )
            #print( beta.props )
            #cat( "init: means are \n" )
            #print( betaMean )
      
            #cat( "init: k is ", beta.k, ",  covs are \n" )
            #print( betaCov )
	...
	## Call the C, C++ code ##
	points <- generateInitPoints.blm( number.draws, X, Y, sigmaDf, sigmaScale, betaMean, betaCov, betaDf, beta.components, beta.props )
...
#
generateInitPoints.blm <- function( number.draws, X, Y, sigmaDf, sigmaScale, betaMean, betaCov, betaDf, beta.components, beta.props, mix.with.MLE = 1 )
{
  ...
  points <- .C( "getInitialPointsBayesLM",
		...
		output.points = as.double( output.points ) )
  ...
  beta.draws <- matrix( points$output.points[ seq(1, dim.Cov * number.draws) ], ncol = number.draws )
  sigma2.draws <- as.vector( points$output.points[ seq( dim.Cov * number.draws + 1, (dim.Cov + 1)* number.draws) ] )
