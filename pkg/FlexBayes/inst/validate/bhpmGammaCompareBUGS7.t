...
prior.distn = bhpm.prior ( xi = xi.prior,
  level2.coef = bayes.normal( zero, identity ), 
  random.var = bayes.invChisq(3, 1),
  common.glm = 2 )
  
# create the chain specifications
sampler.specs <- bhpm.sampler( nSamples=1000, nThin= 500, 
  nChains = 1, nBurnin = 1000,
  init.point = "prior", update.cov = 0 ) 
  
pumps.bhpm <- pumps
pumps.bhpm$ind <- c(rep(1,6), rep(2,4))
...
bhpm.samples <- bhpm( exposure.formula= ~e, random.formula= z~1,
  level2.formula = ~x, group.formula = ~ind,
...
pumps.bugs <- list( e = pumps.bhpm$e, z = pumps.bhpm$z,
  ind = pumps.bhpm$ind, N = Ndata, x = c(-1.0, 1.5) )
...
bugs.samples <- posteriorSamples( data = pumps.bugs,
  parametersToSave = c( "xi", "alpha0", "alpha1", "tau", "beta",
