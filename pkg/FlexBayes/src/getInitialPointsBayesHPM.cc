                     ...
                     double * output_points ) throw( rtErr )
{
  ...
  double beta_df, gamma_df,  alpha_df;
  ...
  //generate fixed effects coefficients from their prior
  if ( fixed_effects )
  {
...
//      alphaInit->setColumn( j, alpha.draw().getVector() );
  ...
  //generate random coefficients variance (if beta prior is not non-informative)
...
//        betaInit[j]->setColumn( i, beta.draw().getVector() );
  ...
  int n_generate;
  if( ( (int) (*common_xi) ) != 3 )  // If there is at least one overdispersion parameter
  {
    ...
    {
      //Uniform Shrinkage
      glmInit = new CMatrix( n_generate, ((int) (*number_draws)) );
      UniformShrinkageDistribution xi( (*glm_pars) );
      for ( j = 0; j < ((int) (*number_draws) ); j++ )
      {
        for ( i = 0; i < n_generate; i++ )
        {
          glmInit->Val( i, j ) = xi.draw().getScalar();
        }
      }  
    }
    else
  ...
  for ( j = 0; j < ((int) (*number_draws) ); j++ )
  {
    ...
    if ( ( (int) (*common_xi) ) != 3 )  // if there is at least one overdispersion parameter
    {
      if ( ( ( (int) (*common_xi) ) == 0 ) || ( ( (int) (*common_xi) ) == 1 ) )
      {
        for ( i = 0; i < ((int) (*number_groups)); i++ )
        {
          output_points[ index ] = glmInit->Val( i, j );
          index++;
        }
      } else {
    	  output_points[ index ] = glmInit->Val( 0, j );
        index++;
      }
    }
