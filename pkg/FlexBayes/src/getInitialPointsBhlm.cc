...
//      gammaInit->setColumn( j, gamma.draw().getVector() );  
  ...
  {
    n_generate = 1;
    multiple_hyper = false; // is false ok?  It needs to be set to something
                            // so if(multiple_hyper) 5 lines down is
                            // well-defined.  WWD, Sept 2010.
  }   
...
//      alphaInit->setColumn( j, alpha.draw().getVector() );
  ...
  //if no non-informative prior for beta
