{cat("---------- BUGS vs. bhlm: Linear Regression T Prior ----\n");T}
{
...
bugsModel <- function (){
  ...
  beta[1:4] ~ dmt(beta.mean[], beta.prec[,], beta.df)
...
stackModelData$beta.df <- beta.df
...
coefPrior <- bayes.t(mean.vector=beta.mean, 
  covmat = signif( solve( beta.prec ) ), df=beta.df)
...
compareSampleDistributions(getSamples( bhlmSamples, c((1:4),6) ), 
